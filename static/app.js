...
const state={catalog:[],design:null,result:null,selected:null,mode:'specimen',history:[],historyIndex:-1,projectId:null,dirty:false,busy:false,pending:null,hidden:new Set(),status:null};
const symbols={round_specimen:'⌁',flat_specimen:'▱',wafer:'◯',link:'⊙',plate:'▦',bracket:'∟',cylinder:'◉',extrusion:'⬡',robot_arm:'⌁'};
const subtitles={round_specimen:'목이 얇은 원통형 · 매끄러운 곡선 전이',flat_specimen:'평판형 인장 시편 · 평행부와 그립',wafer:'실리콘 웨이퍼 · 원판과 플랫',link:'양단 관통 구멍 · 연결 링크',plate:'기계 부품 · 직사각형 구멍판',bracket:'직각 브래킷 · 두 면의 관통 구멍',cylinder:'원통과 튜브 · 회전 부품',extrusion:'좌표 기반 스케치 · 다각형 돌출',robot_arm:'베이스 · 두 링크 · 연결 핀'};
let viewer,toastTimer,sketchEditor;
...
function persist(){try{localStorage.setItem('prompt-cad-last',JSON.stringify({project:project(),id:state.projectId}));}catch{toast('브라우저 자동 저장 공간이 부족합니다. 프로젝트 저장을 사용하세요.',true);}}
function project(){return {format:'prompt-cad-project',version:1,design:clone(state.design),prompt:$('prompt').value};}
...
function commit(result,label,{reset=false,record=true,fit=true}={}){
  ...
  if(reset){state.history=[];state.historyIndex=-1;}
  if(record&&JSON.stringify(state.history[state.historyIndex]?.design)!==JSON.stringify(state.design)){state.history=state.history.slice(0,state.historyIndex+1);state.history.push({label,design:clone(state.design)});if(state.history.length>30)state.history.shift();state.historyIndex=state.history.length-1;}
...
function renderInspector(){
  ...
  $('sketch-fields').hidden=p.geometry.kind!=='extrusion';
  ...
  $('geometry-note').textContent=p.geometry.kind.includes('specimen')?'전이부는 3차 베지어 곡선입니다. 특정 시험 규격의 인증·물성 해석은 포함하지 않습니다.':p.geometry.kind==='extrusion'?'외곽점은 순서대로 연결됩니다. 구멍과 외곽선의 교차를 검사한 후 실제 솔리드로 돌출합니다.':'부품의 로컬 치수와 월드 위치를 편집합니다. 조립 구속으로 기준점과 관절을 연결할 수 있습니다. 부모 치수·각도 변경 시 자식 배치도 갱신됩니다.';
...
async function applyDimensions(){
  ...
  try{
    if(p.geometry.kind==='extrusion'){p.geometry.points=parseRows($('sketch-points').value,2);p.geometry.holes=parseRows($('sketch-holes').value,3);}
...
function renderTemplates(){
  ...
  for(const item of state.catalog.filter(x=>x.mode===state.mode)){
    const button=el('button','template-card');button.append(el('span','template-icon',symbols[item.kind]));const text=el('span',null,item.title);text.append(el('small',null,item.kind==='robot_arm'?'5개 부품 · 관절 구속 4개':item.kind==='extrusion'?'다각형 · 자유 좌표':'매개변수 형상'));button.append(text);button.onclick=()=>guard(()=>newDesign(item));list.append(button);
  }
}
...
function renderTimeline(){
  const line=$('timeline');line.replaceChildren();state.history.forEach((item,i)=>{const b=el('button',(i===state.historyIndex?'current':'')+(i>state.historyIndex?' future':''),i===0?'◈':'◇');b.title=`${i+1}. ${item.label}`;b.setAttribute('aria-label',`이력 ${i+1}: ${item.label}`);b.onclick=()=>guard(()=>goHistory(i));line.append(b);});
  $('timeline-info').textContent=state.history[state.historyIndex]?.label||'';
}
async function goHistory(index){if(!requireReady()||index<0||index>=state.history.length)return;const before=state.historyIndex;state.historyIndex=index;try{await build(clone(state.history[index].design),'',{record:false});}catch(e){state.historyIndex=before;throw e;}}
async function save(){
...
async function exportModel(fmt){
  ...
  try{const r=await fetch('/api/export/'+fmt,{method:'POST',headers:{'Content-Type':'application/json','X-CAD-Request':'1'},body:JSON.stringify(state.design)});if(!r.ok){const error=await r.json();throw new Error(error.detail||'내보내기에 실패했습니다.');}download(await r.blob(),filename(fmt==='autodesk'?'autodesk.zip':fmt));toast(`${fmt.toUpperCase()} 파일을 내보냈습니다.${fmt==='stl'?' 불러올 때 단위를 mm로 지정하세요.':''}`);}finally{busy(false);}
}
...
async function restore(project,id=null){busy(true,'프로젝트 복원 중');try{const result=await api('/api/projects/validate',project);state.projectId=id;state.selected=null;state.hidden.clear();$('prompt').value=result.project.prompt;commit({design:result.project.design,...result},'프로젝트 복원',{reset:true});toast('프로젝트를 복원했습니다.');}finally{busy(false);}}
...
async function boot(){
  ...
  let recovered=false;try{const saved=JSON.parse(localStorage.getItem('prompt-cad-last'));if(saved?.project){await restore(saved.project,saved.id);recovered=true;}}catch{toast('이전 자동 저장을 복원하지 못해 기본 시편을 엽니다.',true);}
  ...
  $('project-name').addEventListener('change',()=>guard(async()=>{if(!requireReady())return;await ensureApplied();const d=clone(state.design);d.name=$('project-name').value.trim();await build(d,'프로젝트 이름 변경',{fit:false});}));
  ...
  bind('new-part',()=>{if(requireReady())$('library-dialog').showModal();});bind('sketch-tool',()=>addPart(entry('extrusion')));
  ...
  $('provider').addEventListener('change',providerNote);bind('generate-draft',generateDraft);bind('discard-draft',discardDraft);bind('apply-draft',()=>{if(!state.pending)return;const result=state.pending;commit(result,result.provider==='openai'?'AI 설계 적용':'치수 명령 적용');toast('설계 초안을 적용했습니다.');});
  $('import-file').addEventListener('change',()=>guard(async()=>{const file=$('import-file').files[0];if(!file)return;try{if(file.size>1_000_000)throw new Error('설계 파일은 1 MB 이하여야 합니다.');const data=JSON.parse(await file.text());await restore(data);$('projects-dialog').close();}finally{$('import-file').value='';}}));
...
function renderConstraintsPanel(p){
  ...
  (p.features||[]).forEach((feature,i)=>{const row=el('div','constraint-row');row.append(el('span',null,`${i+1}. ${feature.name} · ${feature.operation==='add'?'돌출':'파내기'}`));const edit=el('button',null,'편집');edit.onclick=()=>sketchEditor.open(feature.sketch,{partId:p.id,name:feature.name,feature:clone(feature),featureIndex:i});const remove=el('button',null,'제거');remove.onclick=()=>guard(async()=>{const d=clone(state.design);d.parts.find(x=>x.id===p.id).features.splice(i);await build(d,'면 피처 제거');toast('선택한 피처와 이후에 의존하는 피처를 제거했습니다.');});row.append(edit,remove);panel.append(row);});
...
async function startFaceSketch(id,face){if(!requireReady())return;if(!face?.planar)throw new Error('곡면에는 직접 스케치를 만들 수 없습니다. 평평한 면을 선택하세요.');await ensureApplied();viewer.facePick=false;$('face-sketch').classList.remove('active');state.selected=id;renderTree();renderInspector();const part=selectedPart();if((part.features||[]).length>=8)throw new Error('부품당 면 스케치 피처는 최대 8개입니다.');const sketch={kind:'extrusion',thickness:5,points:[{x:-8,y:-8},{x:8,y:-8},{x:8,y:8},{x:-8,y:8}],holes:[],constraints:[]};const feature={id:`feature-${Date.now()}`,name:'면 스케치',face:face.index,support_face_count:face.face_count,normal:face.normal,operation:'add',sketch};sketchEditor.open(sketch,{partId:id,name:part.name,face,feature});}
async function applySketch(sketch,context,operation){const d=clone(state.design),part=d.parts.find(p=>p.id===context.partId);if(context.feature){const feature={...context.feature,sketch,operation};part.features??=[];if(context.featureIndex!==undefined)part.features[context.featureIndex]=feature;else part.features.push(feature);}else part.geometry=sketch;await build(d,context.feature?'면 스케치 피처 적용':'스케치 구속 적용');}
