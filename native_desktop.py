...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--open',type=Path);parser.add_argument('--no-restore',action='store_true');parser.add_argument('--smoke-test',type=Path);parser.add_argument('--self-test',type=Path);parser.add_argument('--setup-ai',action='store_true');parser.add_argument('--advanced-smoke',type=Path);parser.add_argument('--reliability-smoke',type=Path);parser.add_argument('--direct-smoke',type=Path);args=parser.parse_args()
    ...
    def report_exception(kind,value,tb):
        ...
        if args.smoke_test or args.advanced_smoke or args.reliability_smoke or args.direct_smoke:(args.smoke_test or args.advanced_smoke or args.reliability_smoke or args.direct_smoke).with_suffix('.error.txt').write_text(text,encoding='utf-8');app.exit(1)
    ...
    window=MainWindow(restore=not (args.no_restore or args.open or args.smoke_test or args.advanced_smoke or args.reliability_smoke or args.direct_smoke));window.show()
