...
def run_smoke(app,window,path):
    ...
    def saved_closed():
        check(not window.sketching and not window.document.design['parts'],'closed sketch can finish without extrusion');identifier=window.document.design['sketches'][0]['id'];window.edit_saved_sketch(identifier,extrude=True);QTest.mouseClick(window.editor.apply_button,Qt.MouseButton.LeftButton)
    def solid():
        check(not window.sketching,'native sketch completion');check(window.result['stats']['valid'],'valid extruded solid');expected=(60*40-math.pi*25)*8;check(abs(window.result['stats']['volume']-expected)<.1,'rectangle and circular hole volume');report['initial_volume']=window.result['stats']['volume'];window.viewport.set_view('top');renderer=window.viewport.renderer;renderer.SetWorldPoint(12,10,8,1);renderer.WorldToDisplay();x,y,_=renderer.GetDisplayPoint();window.viewport.pick(int(x),int(y));check(bool(window.viewport.face and window.viewport.face[1]['planar']),'OpenGL triangle face picking');window.grab().save(str(path.with_name(path.stem+'-solid.png')));window.start_face_sketch();draw('circle',[(15,0),(18,0)]);e=window.editor;e.operation.setCurrentIndex(1);e.depth.setValue(4);e.finish_sketch()
    def saved_face():
        check(not window.sketching,'face sketch can finish before a feature');saved=window.document.design['sketches'][0];check('face' in saved['context'],'saved face sketch retains support plane');window.edit_saved_sketch(saved['id'],extrude=True);check(window.editor.operation.currentData()=='cut','saved face sketch remembers cut operation');QTest.mouseClick(window.editor.apply_button,Qt.MouseButton.LeftButton)
    def pocket():
    ...
    stages.extend([sketch,saved_line,finish_sketch,saved_closed,solid,saved_face,pocket,undone,redone,assembly,driven,specimen,specimen_applied,face_joint,face_joint_applied,workflows_complete,finished])
    def tick():
        try:
