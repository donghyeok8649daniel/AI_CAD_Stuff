...
class MainWindow(QMainWindow):
    def __init__(self,restore=True):
        super().__init__();self.setObjectName('nativeCADMainWindow');self.resize(1500,920);self.setMinimumSize(1080,680)
        self.document=Document();self.result=None;self.selected=None;self.busy=False;self.worker=None;self.sketching=False;self.last_draft=None;self.data_dir=DATA_DIR;self.data_dir.mkdir(parents=True,exist_ok=True);self.autosave=self.data_dir/'native-autosave.cad.json';self.operation_serial=0
        self.stack=QStackedWidget();self.setCentralWidget(self.stack);self.viewport=CADViewport();self.editor=SketchEditor();self.stack.addWidget(self.viewport);self.stack.addWidget(self.editor);self.viewport.part_selected.connect(self.select_part);self.viewport.face_selected.connect(self.face_selected);self.viewport.message.connect(self.message);self.editor.apply_requested.connect(self.apply_sketch);self.editor.cancelled.connect(self.cancel_sketch)
        self.actions={};self.make_menus();self.make_toolbar();self.make_browser();self.make_properties();self.make_timeline();self.make_ai()
        self.progress=QProgressBar();self.progress.setRange(0,0);self.progress.setFixedWidth(140);self.progress.setMaximumHeight(12);self.progress.hide();self.statusBar().addPermanentWidget(self.progress);self.statusBar().addPermanentWidget(QLabel('  mm  |  OCCT · 네이티브 Qt  '));self.message('새 스케치에서 시작하거나 부품을 추가하세요.');self.rebuild_tree();self.show_properties();self.title()
    ...
    def make_menus(self):
        ...
        edit=self.menuBar().addMenu('편집(&E)');edit.addAction(self.action('undo','이전 작업으로',self.undo,'Ctrl+Z','undo'));edit.addAction(self.action('redo','다음 작업으로',self.redo,'Ctrl+Y','redo'));edit.addAction(self.action('delete','선택 부품 삭제',self.delete_part,None,'delete'))
        model=self.menuBar().addMenu('모델링(&M)');model.addAction(self.action('sketch','새 스케치 · XY',lambda:self.start_sketch('XY'),None,'sketch'));model.addAction(self.action('face_sketch','선택 면에서 스케치',self.start_face_sketch,None,'sketch'));model.addAction(self.action('edit_sketch','선택 스케치 편집',self.edit_sketch));model.addSeparator()
        for kind,title in TITLES.items():model.addAction(title,lambda k=kind:self.add_preset(k))
        assembly=self.menuBar().addMenu('조립(&A)');assembly.addAction(self.action('mate','조립 구속 추가…',self.mate_dialog,None,'assembly'))
        ...
        help=self.menuBar().addMenu('도움말(&H)');help.addAction('사용 방법 · 지원 범위',self.help_dialog);help.addAction('이 앱 정보',lambda:QMessageBox.about(self,APP_NAME,'Prompt CAD Studio 2.0\nQt Widgets + VTK OpenGL + Open CASCADE\n\n브라우저와 웹 서버 없이 실행되는 Windows CAD 앱입니다.\n단위: mm\n설계 프로젝트: .cad.json\n형상 교환: STEP / STL'))
    def make_toolbar(self):
        self.toolbar=QToolBar('모델링');self.toolbar.setObjectName('modelToolbar');self.toolbar.setMovable(False);self.toolbar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon);self.toolbar.setIconSize(QSize(27,27));self.addToolBar(self.toolbar)
        for key in ('new','open','save'):self.toolbar.addAction(self.actions[key])
        self.toolbar.addSeparator();self.plane=combo([('XY','XY 평면'),('XZ','XZ 평면'),('YZ','YZ 평면')]);self.toolbar.addWidget(self.plane);self.toolbar.addAction(icon('sketch'),'스케치 작성',lambda:self.start_sketch(self.plane.currentData()));self.toolbar.addAction(self.actions['face_sketch']);self.toolbar.addAction(self.actions['edit_sketch'])
        ...
        b.setMenu(menu);self.toolbar.addWidget(b);self.toolbar.addAction(self.actions['mate']);self.toolbar.addSeparator();self.toolbar.addAction(self.actions['undo']);self.toolbar.addAction(self.actions['redo']);self.toolbar.addAction(self.actions['fit']);self.toolbar.addAction(icon('ai'),'설계 명령',lambda:self.ai_dock.setVisible(not self.ai_dock.isVisible()))
    ...
    def apply_design(self,data,title,context=None,fit=False,cursor=None,after=None):
        ...
        def done(result):
            ...
            if not self.selected or all(p.id!=self.selected for p in d.parts):self.selected=d.parts[0].id
    ...
    def tree_clicked(self,item,column):
        ...
        if data[0]=='feature':self.show_feature(data[2])
    ...
    def tree_edit(self,item,column):
        ...
        if data[0]=='plane':self.start_sketch(data[1])
    ...
    def select_part(self,identifier):self.selected=identifier;self.viewport.select(identifier);self.show_properties()
    ...
    def show_properties(self):
        ...
        if not part:
            self.property_layout.addWidget(label('사람이 설계하고, 필요할 때 AI를 사용하세요.'));self.property_layout.addWidget(label('① 기준 평면 선택\n② 스케치 작성\n③ 닫힌 영역 돌출\n④ 면 선택 → 스케치 → 구멍 / 돌출',True));self.property_layout.addWidget(button('XY 평면에 스케치',lambda:self.start_sketch('XY'),True));self.property_layout.addWidget(button('구멍판으로 시작',lambda:self.add_preset('plate')));self.property_layout.addStretch();return
        ...
        self.property_layout.addLayout(tform);fixed=QCheckBox('조립 기준 부품으로 고정');fixed.setChecked(part['fixed']);self.property_layout.addWidget(fixed)
        def apply():
            if self.busy:return
    ...
    def delete_part(self):
        ...
        data=deepcopy(self.document.design);data['parts']=[p for p in data['parts'] if p['id']!=self.selected];data['mates']=[m for m in data['mates'] if self.selected not in (m['parent'],m['child'])];self.apply_design(data,'부품 삭제',{'part_id':self.selected},fit=True)
    def start_sketch(self,plane='XY',g=None,context=None):
        if self.busy or self.sketching:return
        context=context or {'plane':plane,'title':f'새 스케치 · {plane} 기준 평면'};self.sketching=True;self.editor.start(g,context);self.stack.setCurrentWidget(self.editor);self.property_dock.hide();self.ai_dock.hide();self.browser_dock.hide();self.timeline_dock.hide();self.toolbar.hide();self.set_busy(False)
    ...
    def cancel_sketch(self):
        if self.busy:return
        self.sketching=False;self.editor.solve_timer.stop();self.editor.revision+=1;self.stack.setCurrentWidget(self.viewport);self.toolbar.show();self.browser_dock.show();self.property_dock.show();self.timeline_dock.show();self.set_busy(False);self.show_properties();self.viewport.window.Render()
    ...
    def rebuild_timeline(self):
        ...
        for n,e in enumerate(journal.path()):
            ...
            if e['id']==journal.data['cursor']:item.setBackground(QColor('#cae5dc'));self.timeline.setCurrentItem(item)
    ...
    def show_mate(self,identifier):
        mate=next(m for m in self.document.design['mates'] if m['id']==identifier);clear_layout(self.property_layout);self.property_layout.addWidget(label('조립 구속 · '+mate['kind']));self.property_layout.addWidget(label(json.dumps(mate,ensure_ascii=False,indent=2),True));self.property_layout.addWidget(button('구속 / 관절 값 편집',lambda:self.mate_dialog(identifier),True))
        def remove():data=deepcopy(self.document.design);data['mates']=[m for m in data['mates'] if m['id']!=identifier];self.apply_design(data,'조립 구속 삭제',{'mate_id':identifier})
    ...
    def new_document(self):
        ...
        self.document=Document();self.result=None;self.selected=None;self.last_draft=None;self.viewport.load(None);self.viewport.hidden.clear();self.rebuild_tree();self.rebuild_timeline();self.show_properties();self.title();self.autosave.unlink(missing_ok=True)
    ...
    def closeEvent(self,event):
        ...
        self.autosave_document();self.editor.solve_timer.stop();self.viewport.shutdown();event.accept()
