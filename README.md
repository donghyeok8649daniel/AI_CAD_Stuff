...
![스케치 도구와 구속](docs/sketch-tools.png)
...
- **스케치 돌출:** 선·원·호·타원·스플라인·텍스트로 스케치하고 닫힌 영역을 선택합니다. 기존 다각형 프로젝트도 열 수 있습니다.
...
- **작업 이력:** 하단 **기록 · 피처**에서 적용한 작업, 도구 사용 순서, 좌표·치수 변경 전후, 면·피처·구속 참조를 확인합니다. 특정 단계 형상으로 이동할 수 있고, 그 단계에서 수정하면 기존 이후 기록을 보존하는 분기가 생깁니다. 브라우저 IndexedDB, 프로젝트 저장, CAD JSON 내보내기에 기록을 포함합니다. 단계 수를 30개로 잘라내지 않으며 파일은 최대 32 MB입니다. 이전 버전에서 남기지 않은 과거 작업은 소급 복원하지 않습니다.
...
**면 스케치:** 도구를 누르고 3D 모델의 평평한 면을 클릭합니다. 면 경계를 참조하며 도형을 그리고, 돌출 또는 안쪽 파내기를 적용합니다. 하면·측면·회전 부품도 로컬 면 좌표계로 계산합니다. 부품당 최대 8개 피처이며 앞 피처를 제거하면 뒤의 의존 피처도 제거됩니다. 직선·원·원호 경계는 **선택 면의 경계 투영**으로 정확히 복사하고 고정할 수 있습니다. 투영은 생성 당시 형상의 고정 복사이며, 다른 곡선은 회색 참조선으로 표시합니다.

**그리기:** 직선/연속선, 2점·중심·3점 사각형, 중심·2점 지름·3점 원, 중심·3점 원호, 타원, 두 방식의 정다각형, 중심점·중심 슬롯, 맞춤점·제어점 스플라인(열림/닫힘), 점, 텍스트, 구성선을 지원합니다. 안내에 따라 점을 클릭하거나 X/Y 좌표로 입력합니다. 연속선·스플라인은 **그리기 완료** 또는 Enter로 끝냅니다. 요소 선택 후 좌표·반지름·각도 등을 직접 수정할 수 있습니다.

**수정:** 직선·원·원호 자르기, 직선 연장, 직선·원호 분할, 직선 연결 외곽·원·원호 간격띄우기, 두 직선의 모깎기·모따기, 이동·복사·회전·양수 배율, 직선 기준 대칭 복사, 직사각형·원형 패턴을 제공합니다. 텍스트 대칭 복사는 지원하지 않습니다. 위상을 바꾸거나 선택 도형을 변환할 때 연결된 구속을 해제하며 스케치 되돌리기로 복구할 수 있습니다.

**스냅과 원점:** 끝점·중간점·중심점·원 사분점·교점·원점에 스냅합니다. 원점은 사각 표시, 스냅 종류는 캔버스에 표시합니다. 직선/원/원호 교점은 편집 중 계산하며 타원·스플라인 교점은 **구속 해석 · 영역 찾기** 후 CAD 커널 결과를 사용합니다. **스냅 구속**을 켜면 연결점·수평/수직·원점·교점 등의 관계를 구속으로 저장합니다. **점 A를 원점에 고정**은 좌표를 0,0으로 유지하는 구속이며 단순한 커서 스냅과 다릅니다.

**치수·구속:** 고정, 일치, 수평/수직, 거리·수평 거리·수직 거리·각도·반지름·직경, 평행·직각·동일·동심·동일 직선·접선·중간점·대칭·점-곡선·곡률 연속을 해석합니다. A/B 요소와 점, 대칭 기준선 C를 선택합니다. 원점에 중심을 고정하고 직경을 주면 원은 완전 구속됩니다. 치수 구속 목록에서 값을 수정할 수 있으며 자유도·잔차와 충돌 오류를 표시합니다. 일부 접선/점-곡선 관계는 유한 호의 연장 곡선을 기준으로 계산합니다. 스케치당 최대 128개 요소, 160개 구속, 600개 좌표·치수 변수입니다.

**영역 선택:** 교차 곡선과 닫힌 윤곽을 실제 CAD 커널로 분리합니다. 해석 후 돌출할 영역을 체크하세요. 기본은 가장 큰 영역 하나이며 내부 구멍은 제외합니다. 여러 영역 선택 시 분리된 솔리드도 생길 수 있습니다. 구성선·점은 영역을 만들지 않습니다. 스플라인의 그리는 중 미리보기는 해석 후 정확한 커널 곡선으로 교체됩니다.

![작업 기록과 면 참조](docs/design-history.png)
...
면 참조는 피처 생성 당시의 이전 피처 ID·면 인덱스·면 수·법선 (원점·X축 방향도 기록)으로 확인합니다. 앞 형상의 위상이 바뀌면 면을 다시 선택해야 할 수 있으며, 범용 CAD의 영구적인 위상 추적 기능은 아닙니다.
...
앱은 127.0.0.1에만 바인딩합니다. 허용 호스트·동일 출처·전용 요청 헤더·일반 요청 1 MB / 프로젝트·Autodesk 패키지 요청 32 MB 제한·형상 복잡도 제한을 둡니다. 공용 서버 배포를 위한 인증 시스템은 포함하지 않습니다.
...
이 릴리스는 주요 2D 스케치 도구와 구속을 확장한 독립 CAD 앱입니다. Fusion의 모든 스케치 도구·변형·자동 추론과 동등하지는 않습니다. 접선 원 생성, 곡선 슬롯의 일부 변형, 3D 스케치, 교차/곡면 투영, 모든 곡선에 대한 자르기·간격띄우기, 수식 매개변수, 영구 위상 추적은 아직 지원하지 않습니다. F3D/IPT 변환은 STEP 솔리드를 Autodesk 앱에서 저장하는 방식이며 이 앱의 작업 이력을 Autodesk의 편집 가능한 피처 이력으로 변환하지 않습니다.

GPT 없이도 직접 설계 및 **AI 설계 → 로컬 치수 명령 → 초안 생성 → 적용**을 사용할 수 있습니다. 로컬 치수 명령은 규칙 해석이며 학습 모델이 아닙니다. Ollama 같은 PC 내 모델 연결·별도 모델 학습은 이 릴리스에 포함하지 않습니다.
